...
import { imageFilter, singleFileUpload, upload } from '../src/middlewares/fileUpload';
...
import { cloudinaryUpload } from '../src/utils/cloudinary';
  ...
  describe('createUserProfile', () => {
      it('should create a new user profile if user exists and profile does not exist', async () => {
          ...
          expect(userProfileStub.calledOnceWithExactly({ where: { userId: 1 } })).to.be.true;
  ...
  describe('Single File Upload', () => {
      ...
      beforeEach(() => {
        req = { file: { buffer: Buffer.from('test'), originalname: 'test.jpg' } as Express.Multer.File, };
        res = { status: sinon.stub().returnsThis(), send: sinon.stub().returnsThis() };
        next = sinon.spy();
        singleStub = sinon.stub(upload, 'single').callsFake((_fieldName: string) => async (req: Request, _res: Response, cb: any) => {
            await cb(null);
        });
      });
      afterEach(() => {
...
describe('cloudinaryUpload function', () => {
  ...
  it('should upload file to Cloudinary and return secure URL', async () => {
    ...
    const cloudinaryUploadStub = sinon.stub(cloudinary.uploader, 'upload').callsFake((path, callback): undefined => {
...
describe('updateUser', () => {
  ...
  beforeEach(() => {
    ...
    getUserProfileByIdStub = sinon.stub().resolves({ id: 1, username: 'newUsername', email: 'newemail@example.com' });
  });
  ...
  it('should handle case where no fields to update', async () => {
    ...
    expect((res.status as sinon.SinonStub).calledOnceWithExactly(400)).to.be.true;
...
describe('getUser', () => {
  ...
  beforeEach(() => {
    ...
    jsonStub = sinon.stub();
    res = { status: statusStub, json: jsonStub };
    getUserProfileByIdStub = sinon.stub().resolves({ id: 1, username: 'exampleUsername', email: 'example@example.com' });
  });
